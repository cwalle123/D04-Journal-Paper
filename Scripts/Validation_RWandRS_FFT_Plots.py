...
def extract_rs_edges_centerline(
    ...
    _, RS_all_tows_data, _, _ = generate_RS_multitow(
        ...
        tow_toggle_length_mm=tow_length_mm if False else tow_length_mm,  # keep identical behavior
...
def run_fft_compare(
    ...
    mse_rw, rho_rw = compute_mse_and_rho(
        f_ctr_exp, A_ctr_exp, f_ctr_rw, A_ctr_rw, fmax=FMAX_METRICS
    )
    mse_rs, rho_rs = compute_mse_and_rho(
        f_ctr_exp, A_ctr_exp, f_ctr_rs, A_ctr_rs, fmax=FMAX_METRICS
    )
    ...
    print(
        f"  Settings: detrend={USE_DETREND}, window(Tukey a={TUKEY_ALPHA})={USE_WINDOW}, "
        f"padding={USE_PADDING} (PAD_FACTOR={PAD_FACTOR})"
    )
    print(f"  Dominant freq (EXP) [cycles/m]: {dom_f:.6f}")
    print(
        "  RW model:  MSE={:.6e}, rho={}".format(
            mse_rw, "nan" if np.isnan(rho_rw) else f"{rho_rw:.6f}"
        )
    )
    print(
        "  RS model:  MSE={:.6e}, rho={}".format(
            mse_rs, "nan" if np.isnan(rho_rs) else f"{rho_rs:.6f}"
        )
    )
    ...
    def plot_linear(f_exp, A_exp, f_rw, A_rw, f_rs, A_rs):
        ...
        legend = fig.legend(
            handles, labels, loc="lower center", ncol=1, fontsize=font_legend, fancybox=False
        )
...
def write_batch_csv(
    ...
    for tow in range(tow_min, tow_max + 1):
        ...
        if exp_data is None:
            ...
            print(
                f"Tow {tow}: missing/failed (traverse_tow_constructor returned None) -> filling NaNs"
            )
            continue
